...
Description : Run this program to start the AI bin application. It uses the functions in other three programs.
              One cammera is used to do the classfication.
'''
#################################################################################
# logic:
#black screen 
...
def displayImageOnCanvas(display, app,img0):
...
def most_frequent(List):
    ...
    for i in List: 
        ...
        if frequency >count:
            ...
            element = i
    return element
def classiyPrediction(predictions,output,labels,previous=[0,0,0,0,0,0,0]):
    if predictions[output]*100 > 97:
        prediction = labels[output]
    elif output in [1,3] and  predictions[output]*100 > 30:
        prediction = labels[output]
    else:
        prediction="unknown"
    return prediction
...
def classfication(display):
    ...
    while True:
        ...
        if len(queue)<queueLen:
            ...
            break
    output=max(queue)
    prediction=classiyPrediction(predictions,output,labels)
    ...
    while True:
        ...
        # if the user is holding the object
            ...
            var.set("Can not detect anything,pleade put the waste in the centre")
        ...
        displayImageOnCanvas(display,app,img)
        time.sleep(0.05)
        [img,img1]= camera() #take new photo
        starttime=time.time()
        predictions =inference.prediction(img)
        output = np.argmax(predictions)
        print(output)
        ...
        prediction=classiyPrediction(predictions,output,labels,previousPrediction)
        previousPrediction=predictions
    if Label!=None:
        Label.destroy()
